...
function ItemsList(props) {
    ...
    var items = props.items.map(item => {
        ...
        return ( //item.item.name just because it is that way in api
            <div id={x} key={item.itemId} className="item_div">
                ...
                <nobr><Link className="item_link" id={item.itemId} onClick={itemClick} to="/item">{item.item.name} - {item.item.rarity}</Link></nobr>
            </div>
        )
    })
    ...
    return(
        <div className="items_list_div">
            <Link to="/"><h2 className="back">&#x21a9; Go Back</h2></Link>
